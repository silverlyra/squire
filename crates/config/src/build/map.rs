...
impl DirectiveMap {
    ...
    #[must_use]
    pub fn contains<D: Directive>(&self, directive: D) -> bool {
        D::get(self).is_some_and(|existing| existing == directive)
    }
    ...
    pub fn insert<D: Directive>(&mut self, directive: D) -> Option<D> {
        directive.set(self)
    }
    ...
    #[must_use]
    pub fn is_set<D: Directive<Value = ()>>(&self) -> bool {
        D::get(self).is_some()
    }
}
